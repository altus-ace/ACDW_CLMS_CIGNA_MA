CREATE TABLE [adi].[CignaMAMbrElig] (
    [CignaMAMbrEligKey]     INT           IDENTITY (1, 1) NOT NULL,
    [SrcFileName]           VARCHAR (100) NOT NULL,
    [LoadDate]              DATE          NOT NULL,
    [DataDate]              DATE          NOT NULL,
    [CreatedDate]           DATETIME      CONSTRAINT [DF_adiCignaMAMbrElig_CreatedDate] DEFAULT (getdate()) NOT NULL,
    [CreatedBy]             VARCHAR (50)  CONSTRAINT [DF_adiCignaMAMbrElig_CreatedBy] DEFAULT (suser_sname()) NOT NULL,
    [LastUpdatedDate]       DATETIME      CONSTRAINT [DF_adiCignaMAMbrElig_LastUpdatedDate] DEFAULT (getdate()) NOT NULL,
    [LastUpdatedBy]         VARCHAR (50)  CONSTRAINT [DF_adiCignaMAMbrElig_LastUpdatedBy] DEFAULT (suser_sname()) NOT NULL,
...
CREATE TRIGGER [adi].AU_CignaMAMbrElig
ON [adi].CignaMAMbrElig
AFTER UPDATE 
AS
   UPDATE adi.CignaMAMbrElig
   SET [LastUpdatedDate] = SYSDATETIME()
    ,[LastUpdatedBy] = SYSTEM_USER	
   FROM Inserted i
   WHERE adi.CignaMAMbrElig.CignaMAMbrEligKey = i.CignaMAMbrEligKey
   ;
